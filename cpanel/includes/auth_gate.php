      ...
      <form class="auth-form" method="post" action="<?= e(url('/login')) ?>">
        ...
        <label class="auth-line">
          ...
          <span class="auth-line-row">
            <input id="login-username" name="username" type="text" required dir="ltr" autocomplete="username" placeholder="نام کاربری">
        ...
        <div class="auth-line" data-password-field>
          <span>رمز عبور</span>
          <span class="auth-line-row">
            <input id="login-password" name="password" type="password" required dir="ltr" lang="en" autocomplete="current-password" placeholder="رمز عبور" data-password-input>
    ...
    <section class="auth-pane<?= $authMode === 'register' ? ' is-on' : '' ?>" data-auth-pane="register"<?= $authMode === 'register' ? '' : ' inert' ?>>
      <form class="auth-form auth-form-register" method="post" action="<?= e(url('/register')) ?>" id="register-form">
        ...
        <div class="auth-grid">
          <label class="auth-line">
            <span>نام</span>
            <input class="input-rtl" name="first_name" id="first_name" required dir="rtl" autocomplete="given-name" placeholder="نام">
          </label>
          <?php if ($role !== 'DOCTOR'): ?>
          <label class="auth-line">
            <span>نام (انگلیسی)</span>
            <input name="name_en" id="name_en" required dir="ltr" lang="en" autocomplete="off" placeholder="name">
          </label>
          <?php endif; ?>
          <label class="auth-line">
            <span>نام خانوادگی</span>
            <input class="input-rtl" name="last_name" id="last_name" required dir="rtl" autocomplete="family-name" placeholder="نام خانوادگی">
          </label>
          <?php if ($role !== 'DOCTOR'): ?>
          <label class="auth-line">
            <span>نام خانوادگی (انگلیسی)</span>
            <input name="surname" id="surname" required dir="ltr" lang="en" autocomplete="off" placeholder="surname">
          </label>
          <?php endif; ?>
        </div>
        <label class="auth-line">
          <span>نام کاربری</span>
          <?php if ($role === 'DOCTOR'): ?>
            <input name="username" id="username" required dir="ltr" autocomplete="username" placeholder="مثلاً shiva.geran">
        ...
        <label class="auth-line">
          <span>موبایل</span>
          <input name="phone" id="phone" required dir="ltr" inputmode="tel" autocomplete="tel" placeholder="مثلاً 0912... یا +1...">
        </label>
        <?php endif; ?>
        <div class="auth-grid">
    ...
    <div class="auth-band" aria-hidden="true">
      ...
      <div class="auth-band-inner">
        <div class="auth-band-page<?= $authMode === 'login' ? ' is-on' : '' ?>" data-band="login">
          ...
          <h2 class="auth-welcome">خوش آمدید <em>دوباره.</em></h2>
          <p>نوبت‌ها، گفتگوها و پرونده‌تان همان‌جا که گذاشتید منتظر شماست.</p>
        </div>
        <div class="auth-band-page<?= $authMode === 'register' ? ' is-on' : '' ?>" data-band="register">
          <p class="auth-kicker">مانا کلینیک</p>
          <h2 class="auth-welcome">از اینجا <em>شروع کنید.</em></h2>
          <p>یک حساب برای نوبت، کارگاه و ارتباط امن با درمانگر.</p>
        </div>
      </div>
    </div>
  </div>
...
$GLOBALS['pageScripts'] = ($GLOBALS['pageScripts'] ?? '')
    ...
    . '<script src="' . e(url('/assets/js/auth-gate.js')) . '?v=20260910f"></script>';
