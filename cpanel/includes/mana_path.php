...
function mana_path_daily_motto(?string $ymd = null): string
{
    $lines = [
        'امروز لازم نیست کامل باشی؛ فقط یک قدم کوچک کافی است.',
        'نفس بکش. همین لحظه مال توست و از نو شروع می‌شود.',
        'آرامش یک جایزه نیست؛ مهارتی است که امروز هم تمرین می‌شود.',
        'با خودت مهربان باش؛ مسیری که می‌روی ارزش عجله ندارد.',
        'یک کار کوچک امروز، فردا را سبک‌تر می‌کند.',
        'احساسات می‌آیند و می‌روند؛ تو این‌جا می‌مانی.',
        'لازم نیست همهٔ راه را امروز بروی؛ همین قدم را بردار.',
        'درست بودن حال، گاهی یعنی آرام‌تر خواستن.',
        'تو مسئول درمان کامل نیستی؛ مسئول مراقبت امروز هستی.',
        'وقتی خسته‌ای، استراحت هم بخشی از مسیر است.',
        'هر بار که برمی‌گردی، مسیر را از نو می‌سازی.',
        'نگران فردا نباش؛ امروز را با یک کار کوچک جمع کن.',
        'ذهنت را قضاوت نکن؛ فقط تماشایش کن و آرام‌تر برو.',
        'شروع دوباره همیشه ممکن است؛ حتی وسط روز.',
        'تو همان کسی هستی که امروز هم برای خودش وقت گذاشت.',
        'سبک شدن یعنی کمتر جنگیدن با خودت.',
        'یک نفس عمیق، فاصلهٔ بین شتاب و انتخاب است.',
        'پیشرفت آهسته هم پیشرفت است؛ گیاه هم یک‌شبه بزرگ نمی‌شود.',
        'اگر امروز سخت است، سخت بودن دلیل توقف نیست.',
        'همین که این‌جایی، یعنی هنوز انتخاب می‌کنی مراقبت کنی.',
        'لازم نیست قوی به نظر برسی؛ صادق بودن با خودت کافی است.',
        'بدن و ذهن هر دو به مهربانی امروز تو نیاز دارند.',
        'یک تماس کوتاه با خودت، از هزار فکر پراکنده بهتر است.',
        'امروز را مثل یک تمرین کوتاه ببین، نه یک آزمون بزرگ.',
        'جای امن، گاهی همان لحظه‌ای است که به خودت سخت نمی‌گیری.',
        'هر روز یک فرصت تازه است تا کمی آرام‌تر باشی.',
        'کار امروز را ساده بگیر؛ سادگی هم شجاعت می‌خواهد.',
        'تو مجبور نیستی همهٔ بار را یک‌جا برداری.',
        'وقتی آرام می‌شوی، راه خودش را نشان می‌دهد.',
        'حضور داشتن، حتی کم، از غایب بودن بهتر است.',
        'امروز را تمام نکن مگر با یک مهربانی کوچک به خودت.',
    ];
    $key = $ymd ?: mana_path_today_ymd();
    $i = abs(crc32($key)) % count($lines);
    return $lines[$i];
}
