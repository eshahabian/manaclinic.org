...
if ($appointmentId === '') {
    ...
    redirect('/doctor/patients/' . $patientId . '?tab=' . $nextTab);
}
...
if ($row) {
    $pdo->prepare('UPDATE doctor_session_notes SET note_text=?, s_text=?, d_text=?, a_text=?, p_text=? WHERE id=? AND doctor_id=?')
        ->execute([$noteText, $sText, $dText, $aText, $pText, $row['id'], $doctorId]);
} else {
    $pdo->prepare('INSERT INTO doctor_session_notes (id, doctor_id, patient_id, appointment_id, note_text, s_text, d_text, a_text, p_text) VALUES (?,?,?,?,?,?,?,?,?)')
        ->execute([cuid(), $doctorId, $patientId, $appointmentId, $noteText, $sText, $dText, $aText, $pText]);
}

flash_set('success', 'شرح حال این جلسه (SOAP) ذخیره شد.');
$anchor = 'session-' . $appointmentId;
redirect('/doctor/patients/' . $patientId . '?tab=' . $nextTab . '#' . rawurlencode($anchor));
