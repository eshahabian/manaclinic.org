        ...
        if (!$exists->fetch()) {
            $pdo->prepare('INSERT INTO articles (id,title,slug,content,excerpt,published,published_at,author_id) VALUES (?,?,?,?,?,1,NOW(),?)')
                ->execute([
                    ...
                    'اضطراب بخشی طبیعی از زندگی است؛ اما با تنفس آگاهانه، نظم روزانه و حمایت تخصصی می‌توان فشار آن را کم کرد — بدون اینکه جای درمان فردی را بگیرد.',
                    $author['id'],
                ]);
        }
        ...
        if (!$exists->fetch()) {
            $pdo->prepare('INSERT INTO articles (id,title,slug,content,excerpt,published,published_at,author_id) VALUES (?,?,?,?,?,1,NOW(),?)')
                ->execute([
                    ...
                    'خواب کافی پایهٔ تمرکز، خلق پایدار و آرامش ذهن است؛ بی‌خوابی می‌تواند اضطراب را شدیدتر کند — بدون اینکه جای ارزیابی تخصصی را بگیرد.',
                    $author['id'],
                ]);
        }
